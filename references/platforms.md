...
- ⚠️ **React 受控组件输入技巧**：不能直接 `fill` 或设 `input.value`，必须用以下方式之一：
  - **首选**：`snapshot -i` 获取 ref → `click "@ref"` 聚焦 → `type "搜索词"` 直接输入
  - **备选**：`nativeInputValueSetter`（见下方脚本）
```js
...
- 搜索按钮：**优先使用 snapshot ref 点击**（按钮文本可能包含空格/不可见字符，JS eval 文本匹配不稳定）
  - 步骤：`agent-browser snapshot -i` → 找到搜索按钮的 ref → `agent-browser click "@eXX"`（⚠️ ref 必须用引号包裹）
  - 备选方案：JS eval 点击
```js
// 备选：JS eval 点击（按钮文本匹配可能不精确）
const btns = document.querySelectorAll('button');
for(const b of btns) { if(b.textContent.trim() === '搜索') { b.click(); break; } }
```
...
**方案 A（首选）：Ctrl+A 全选替换**
- `browser_click(ref_search_input)` → `browser_press_key(ref, "Control+a")` → `browser_type(ref, "新搜索词")` → 点击搜索按钮
- 优点：不重新加载页面，不触发反爬
- 缺点：依赖 React 正确响应 selectAll 事件

**方案 B：JS nativeInputValueSetter**
...
**⚠️ PowerShell 下 `eval` 的分号截断问题：**

PowerShell 中执行 `agent-browser eval "xxx; yyy"` 时，分号后面的内容会被截断，导致 SyntaxError。

**解决方案：用 PowerShell 变量读取文件再传参**

```powershell
# 1. 将 JS 代码保存为 .js 文件（UTF-8 编码）
# 2. 用 PowerShell 变量读取文件内容
...
- ✅ 必须用上述 PowerShell 变量中转方式

**内置提取脚本应预置为独立 .js 文件**，而非在命令行中内联含分号的代码。
...
- 提取脚本见上方「完整提取脚本」
- ⚠️ **选择器失效自动检测**：提取脚本执行前先检查选择器是否存在：
  ```js
  // 如果主选择器不存在，说明页面结构已变化
  if (!document.querySelector('.resumeCardWrap--FcnzW')) {
    // 回退到 snapshot 方式提取（Level 2）
  }
  ```

**Level 2（降级）：snapshot 文本解析 + get_attribute 补充**
- 适用：MCP browser（无 JS eval）或选择器失效时
...
**选择器降级策略：**
- 主选择器（如 `.resumeCardWrap--FcnzW`）失效时，回退到通用选择器
- 搜索框降级：`.searchInput--KgDn1` → `input[type="text"]` → snapshot ref
- 候选人卡片降级：`.resumeCardWrap--FcnzW` → snapshot 文本解析
- CSS 选择器中的 hash 后缀（如 `--FcnzW`）可能在猎聘发版后变化
...
9. **`click @ref` 必须用引号包裹**：`agent-browser click "@e13"` ✅ / `agent-browser click @e13` ❌（报 "Missing arguments for: click"）
10. **猎聘反爬敏感**，短时间内连续导航 2 次以上会触发验证码（`safe.liepin.com`），优先用 Ctrl+A 替换而非导航重置
...
4. PowerShell 中 `run-code` 的分号会被截断，复杂 JS 用脚本文件或 PowerShell 变量中转（详见 `references/windows.md`）
